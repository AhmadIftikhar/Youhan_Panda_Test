...
Shader "UChromaKey/Transparency mask/Direct source/Unlit transparent" {
Properties {
	_MainTex ("Video source (RGB)", 2D) = "white" {} 
	_MaskTex ("Mask texture(RGB)", 2D) = "black" {}
	_CKCol ("Mask transparent color", Color) = (0,0,0,1)	 
		...
		CGPROGRAM
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				{					
					...
					fixed4 mask = tex2D(_MaskTex, nuv);
					
					c.a = (1 - saturate((1 - ((mask.r - _CKCol.r)*(mask.r - _CKCol.r) + (mask.g - _CKCol.g)*(mask.g - _CKCol.g) + (mask.b - _CKCol.b)*(mask.b - _CKCol.b)) / (_Range * _Range)) * _EdgeSharp) * _Opacity);
					return c;
				}
				else
				{
